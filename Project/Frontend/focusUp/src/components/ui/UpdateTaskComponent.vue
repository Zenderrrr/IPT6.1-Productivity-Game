...
const emit = defineEmits<{
  (e: 'submit', taskId: number, task: UpdateTask) : void
  (e: 'cancel') : void
}>()
...
function submit(){
  ...
  emit('submit', props.task.id, task)
}
...
<template>
  <PopUpWindow @cancel="cancel" title="Bearbeite deine Task">
      <!-- Inputs-->
      <form @submit.prevent="submit" class="w-full flex flex-col gap-5 mt-6">
        ...
        <div class="flex items-start justify-center gap-3 w-full">
          ...
          <div class="flex items-start justify-center flex-col gap-1.5 w-full">
            ...
            <input v-model="dueDate" class="input-hover cursor-pointer w-full px-3 py-1 border border-gray-200 rounded-lg outline-[var(--primary-color)]" type="date" id="title">
          </div>
        </div>
        ...
        <div class="w-full flex justify-end items-center gap-3">
          ...
          <button type="submit" class="scale-animation-sm cursor-pointer px-4 py-2 bg-linear-to-r from-[var(--primary-color)] to-[var(--secondary-color)] border rounded-lg text-[var(--text-color-white)] font-semibold">Task bearbeiten</button>
        </div>
      </form>
